...
SAVE_PATH = "C:\\Users\\Gabriel Schwarzbauer\\Desktop\\Aktienai_eine_Woche_vorhersage\\AktienInformationDownloader\\12_3"  # Passe den Pfad nach Bedarf an
os.makedirs(SAVE_PATH, exist_ok=True)  # Erstelle den Ordner, falls er nicht existiert

# Hole die letzten 15 Jahre als möglichen Zeitraum
end_date = datetime.date.today()
start_date = end_date - datetime.timedelta(days=15 * 365)

# Erstelle eine Liste mit möglichen Daten
date_range = [start_date + datetime.timedelta(days=i) for i in range(((end_date - start_date).days) - 15)]
...
for _ in range(num_loops):
    ...
    if stock_data.empty:
        ...
        filtered_data = stock_data["Close"].head(15)
        
        if len(filtered_data) >= 15:
            filename1 = os.path.join(SAVE_PATH, f"{TICKER}_random_15_trading_days.txt")
            ...
            filename2 = os.path.join(SAVE_PATH, f"{TICKER}_prices_only_15.txt")
            prices = [f"{float(price):.8f}" for price in filtered_data.values]
            prices_first_12 = ",".join(prices[:12])
            prices_last_3 = ",".join(prices[12:])

            file_mode = "a" if os.path.exists(filename2) else "w"
            with open(filename2, file_mode) as file:
                file.write(prices_first_12 + "\n")
                file.write(prices_last_3 + "\n")
