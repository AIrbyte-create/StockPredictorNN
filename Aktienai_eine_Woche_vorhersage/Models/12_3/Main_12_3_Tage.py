...
model = keras.Sequential([
    ...
    layers.Dense(3)  # Jetzt 3 Werte als Ausgabe
])
...
model_save_path = os.path.join(BASE_DIR, r"Aktienai_eine_Woche_vorhersage/Models/12_3/trained_model.h5")
...
def load_model():
    if os.path.exists(model_save_path):
        ...
        model = keras.Sequential([
            layers.Dense(32, activation='relu', input_shape=(12,)),
            layers.Dense(16, activation='relu'),
            layers.Dense(3)  # 3 Werte als Ausgabe
        ])
        model.compile(optimizer='adam', loss='mean_squared_error')
...
def train_model_with_user_data():
    ...
    while True:
        # Benutzereingaben
        print("\n")
        user_input = input("Gib 12 Zahlen für die Eingabedaten (durch Kommas getrennt) ein oder 'exit' zum Beenden: ")
        ...
        try:
            ...
            if len(user_data) != 12:
                print("\n")
                print("Bitte genau 12 Zahlen eingeben.")
                continue
            print("\n")
            next_numbers = list(map(float, input("Gib 3 Zielwerte für die nächste Ausgabe ein (durch Kommas getrennt): ").split(',')))
            if len(next_numbers) != 3:
                print("Bitte genau 3 Zielwerte eingeben.")
                continue
            ...
            print("\n")
            print(f"Vorhergesagte nächste 3 Zahlen (ohne Training): {prediction_before[0]}")
            ...
            print("\n")
            print(f"Vorhergesagte nächste 3 Zahlen (mit Training): {prediction_after[0]}")
...
# Funktion, um das Modell zu testen (Vorhersage der nächsten 3 Zahlen)
def test_model():
    print("\n")
    test_input = input("Gib 12 Zahlen ein, um die nächsten 3 Zahlen vorherzusagen (durch Kommas getrennt): ")
    ...
    try:
        ...
        # Vorhersage der nächsten 3 Zahlen
        prediction = model.predict(np.array([user_data], dtype=np.float32))
        print("\n")
        print(f"Vorhergesagte nächsten 3 Zahlen: {prediction[0]}")
        
        # Korrigierte Ausgabe mit float-Wert statt Array
        if np.all(prediction[0] > user_data[-3:]):
            print(f"INVEST: Vorhersage = {prediction[0]}, Letzte 3 Zahlen = {user_data[-3:]}")
        else:
            print(f"DON'T INVEST: Vorhersage = {prediction[0]}, Letzte 3 Zahlen = {user_data[-3:]}")
...
def plot_differences(differences):
    ...
    directory = os.path.join(BASE_DIR, r"Aktien_Trainer/Lern-Fortschritt/12_3")
